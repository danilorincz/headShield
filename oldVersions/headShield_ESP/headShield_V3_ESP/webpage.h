...
<! USER DATA------------------------->
  <div class="card">  
    ...
    <h2> Pressure: <span style="color:yellow" id="sinceStart">0</span> [hh:mm:ss]</h2>
    <h2> Temperature: <span style="color:yellow" id="batteryCharge">0</span> %</h2>
    <h2> Humidity: <span style="color:yellow" id="fanSpeed">0</span> %</h2>
    <h2> CO2: <span style="color:yellow" id="deltaPressure">0</span> Pa</h2>
  </div>
...
<!-------------------------JavaScrip------------------------->
  <script>
    ...
    {
      var fanSpeedRequest = new XMLHttpRequest();
      fanSpeedRequest.onreadystatechange = function()
      {
        if(this.readyState == 4 && this.status == 200)
        {
          document.getElementById("fanSpeed").innerHTML =
          this.responseText;
        }
      };
      fanSpeedRequest.open("GET", "readFanSpeed", true);
      fanSpeedRequest.send();
    }
    // AJAX->DELTA PRESSURE--------------------------------
    setInterval(function()
    {
...
<! USER DATA------------------------->
  <div class="card">  
    <h1> <span style="background-color:white">Usefull data</span> </h1>
    ...
    <h2> Fan speed: <span style="color:yellow" id="fanSpeed">0</span> %</h2>
    <h2> Delta pressure: <span style="color:yellow" id="deltaPressure">0</span> Pa</h2>
    <h2> Status: <span style="color:yellow" id="statusState">0</span> Pa</h2>
  </div>
...
<!-------------------------JavaScrip------------------------->
 ...
 // AJAX->SINCE START--------------------------------
    ...
    function getStatusState()
    {
      ...
      statusStateRequest.open("GET", "readStatusState", true);
